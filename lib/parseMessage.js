...
function getConceptsList() {
  return concepts.map((c, i) => `${i + 1}. ${c.label} (${c.code})`).join('\n');
}
...
function parseDateInput(text) {
  ...
  return { dateStr, month: d.month, year: d.year };
}

module.exports = { parseMessage, rowToSheetRow, getConceptsList, parseBulkLines, parseDateInput };